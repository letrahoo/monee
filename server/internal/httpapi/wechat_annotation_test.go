...
func TestWeChatAndAnnotationEndpoints(t *testing.T) {
	for _, format := range []struct {
		...
		t.Run(format.extension, func(t *testing.T) {
			...
			raw, e := os.ReadFile("../../../fixtures/synthetic/wechat/ordinary-and-review." + format.extension)
			if e != nil {
				t.Fatal(e)
			}
			b, _ := json.Marshal(map[string]string{"filename": "wx." + format.extension, "account": "wallet", "contentBase64": base64.StdEncoding.EncodeToString(raw)})
			...
			if w.Code != 200 || json.Unmarshal(w.Body.Bytes(), &sources) != nil || len(sources) != 1 || sources[0].Line != format.line {
				t.Fatal(w.Body.String())
			}
