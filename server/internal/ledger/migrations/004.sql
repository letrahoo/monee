-- Rebuild on a pinned connection with foreign keys disabled outside the
-- transaction; validate every FK before committing, as for the v3 migration.
CREATE TABLE transactions_v4 (
 id TEXT PRIMARY KEY, ledger_id TEXT NOT NULL REFERENCES ledgers(id), occurred_on TEXT NOT NULL,
 kind TEXT NOT NULL CHECK(kind IN ('expense','income','refund','transfer')), amount_minor INTEGER NOT NULL,
 ...
 created_by TEXT REFERENCES app_users(id), refund_of TEXT, to_account TEXT,
 CHECK((kind='expense' AND amount_minor<0) OR (kind IN ('income','refund','transfer') AND amount_minor>0)),
 CHECK((kind='refund' AND refund_of IS NOT NULL AND refund_of<>id) OR (kind<>'refund' AND refund_of IS NULL)),
 CHECK((kind='transfer' AND to_account IS NOT NULL AND length(trim(account))>0 AND length(trim(to_account))>0 AND account<>to_account) OR (kind<>'transfer' AND to_account IS NULL)),
 UNIQUE(ledger_id,identity_key), UNIQUE(ledger_id,id),
 FOREIGN KEY(ledger_id,refund_of) REFERENCES transactions_v4(ledger_id,id)
);
INSERT INTO transactions_v4 (
 ...
 identity_key,fingerprint,similarity_key,version,device_id,created_at,updated_at,deleted_at,created_by,refund_of
) SELECT
 id,ledger_id,occurred_on,kind,amount_minor,currency,merchant,category,source,account,external_id,note,
 identity_key,fingerprint,similarity_key,version,device_id,created_at,updated_at,deleted_at,created_by,refund_of
FROM transactions;
