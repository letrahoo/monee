...
- 所有功能变更都必须走完整验证流程，即使只改后端：后端测试与静态检查、共享客户端测试、Web 生产构建、Mac 分发构建，以及两端针对受影响流程的实际操作验证。构建成功不能代替运行验收。
- 两端验证覆盖主流程及相关异常、权限、重复操作与数据一致性；使用独立开发账本和合成资料，不在正式账本试写。不得以绕过登录方式补足验收。
- PR 必须记录所验证提交、环境、用例、实际结果和未完成项。遇到凭据、交互工具或平台阻塞时继续完成可执行检查，明确标记未验收，不能宣称功能已全部验证。
- “不做模型试跑”仅约束云端模型调用，不免除代码、构建和两端验证；收费模型调用遵循另行批准的预算。
