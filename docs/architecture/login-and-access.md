...
KMP JVM 测试、Web 生产构建与 Mac .app 构建通过。隔离浏览器使用合成提供方和合成账本，验证未授权提示、Google 超管邮箱绑定、添加成员、成员访问和停用拦截，以及服务断开时隐藏账本和显示连接错误、服务恢复后自动重新验证权限。实际 GitHub App 已验证可跳转到其官方登录页面；完整回调仍需用户在浏览器完成账号登录和授权。Google 仍需 Web OAuth 客户端配置。Mac 的系统浏览器授权领取链路由接口测试覆盖；更新后的原生界面与真实提供方回调仍待人工验收。
