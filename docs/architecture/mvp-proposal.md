...
Mac 通过 Compose Desktop 的原生分发打包。Go 服务可作为附带可执行程序，由桌面平台层管理；需要完成单实例锁、发现已有实例、启动失败处理、架构打包、退出生命周期与更新测试。当前开发运行方式先启动独立 Go 服务，Mac 读取私有连接文件发现实例；已实现单实例锁和凭据轮换。将 Go 服务随 .app 打包并自动管理其生命周期仍待实施。
