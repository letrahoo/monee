...
状态：KMP + Go + SQLite 的首条数据链路已实现，其余业务为后续提案。更新：2026-09-14。

已交付范围及当前 API 以[本地数据链路](local-data-flow.md)和 [OpenAPI](../../api/openapi.yaml) 为准。本文中的目录、模型与 API 草案用于完整 MVP 演进，不是当前功能清单。

对应需求：[MVP 产品讨论稿](../product/mvp-discussion.md)。初始仓库基线 `b3ebbb2` 仅包含 README 和 MIT License；当前已建立 Web/Mac 共用界面，接入 Go API 与 SQLite，完成标准 CSV 预览/确认、手动收支、去重和月度查询。
...
第一轮曾使用合成数据验证中文、金额、筛选、详情、图片资源和响应式布局；当前默认页面已改为读取本地持久化账本。Compose Web 的浏览器能力、包体与加载性能需实测，平台支持以验证记录为准。
...
用户已选择邮件附件/指定文件夹优先，手动导出补充。当前已先完成手选标准 CSV 的处理闭环。接收自动化首先实现指定文件夹，随后按实际邮箱服务适配邮件入口。
...
Mac 通过 Compose Desktop 的原生分发打包。Go 服务可作为附带可执行程序，由桌面平台层管理；需要完成单实例锁、发现已有实例、启动失败处理、架构打包、退出生命周期与更新测试。当前开发运行方式先启动独立 Go 服务，Mac 读取私有连接文件发现实例；已实现单实例锁与不含凭据的服务发现；Web / Mac 均通过 Google / GitHub 登录并由后端白名单控制账本访问，详见 [登录与授权](login-and-access.md)。将 Go 服务随 .app 打包并自动管理其生命周期仍待实施。
...
上面仍为完整 MVP 草案。当前已提供 [OpenAPI](../../api/openapi.yaml)，使用 dashboard、imports/preview 等首轮接口，包含错误、分页、金额、确认和鉴权契约；未实施的草案接口不可调用。
