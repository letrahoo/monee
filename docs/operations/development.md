...
服务首次启动在私有数据目录生成 `redaction.key`（32 字节随机值，0600；数据目录需为 0700）。它用于生成同账本内稳定的关联标识，与模型 API Key 不同；接口和日志不会返回它。文件损坏、权限不正确或为软链接时只关闭脱敏预览，其他账本功能仍可用；不要通过删除密钥来处理错误。

密钥和账本应通过可信加密备份一同保管，不得提交 Git 或放入 CI。当前数据库快照工具不会自动打包此文件；迁移需另外安全恢复，并保持权限。丢失密钥后会生成不同关联标识，无法延续先前别名；本轮没有自动轮换/恢复界面。

导入批次的“查看脱敏预览”仅在本机生成白名单字段。日期保留到日，金额保留整数分，账户/商户/订单号用 HMAC 引用，商品/备注等原文不进入 payload；无效记录单列排除原因。投影来自原始来源快照，不替代当前账本交易，也不代表已授权向云端发送。没有模型请求、提供方设置或收费调用。
...
网关必须把 Mac `Authorization: Bearer` 交给 Monee 服务验证；清理历史外层密码时只能移除遗留 Basic 头，不能删除全部 Authorization。网关修改需遵循实际审批；本机修复已于用户明确批准后应用并重载。域名能显示登录页不代表完整登录/记账已通过。功能验收必须切到经授权的独立候选账本，不能试写原服务真实数据。
