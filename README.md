...
已打通 **KMP Web / Mac → Go API → 本机 SQLite**。两端读取同一份账本，支持标准 CSV 的预览与确认导入、重复流水检查、手动收入/支出、月度统计、中文搜索、分页和明细。默认打开真实空账本，确认后持久化，客户端每 5 秒刷新。
...
```sh
# 先启动本地 Go 服务，同时提供 Web 页面。保持此终端运行。
...
Mac 数据默认保存在 `~/Library/Application Support/Monee/`，应用重建不会清空账本。可通过 `MONEE_DATA_DIR` 为服务和 Mac 指定同一个私有目录。不要把账本目录放进仓库；也不要用独立静态文件服务替代上述启动方式，Web 需要与 API 同源。停止服务后，页面会提示连接失败并标明旧数据。
